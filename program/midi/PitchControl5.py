...
while EndTrigger:
    for notes in lines:
        time_stamp += long(notes[-1])/1.85
        Ftime_stamp += long(notes[-1])/1.85
        if Ftime_stamp >= 5000:
            time.sleep(5)
            Ftime_stamp = 0
        if time.time() - Mtime > 13:
            pitch -= 1
            Mtime = time.time()
            print("pitch up!")
        ...
        #note_on event
        if notes[0] == 'non':
            player.write([[[0x90 + int(notes[1]),int(notes[2]) + pitch,int(notes[3])],time_stamp]]) #信号ステータス,ピッチ,ベロシティ,タイムスタンプ
