...
EndTrigger = True       #音源ループトリガー
Stime = time.time()

while EndTrigger:
    for notes in lines:
        #-----テンポ調整&再生時間計測-----
        time_stamp += long(notes[-1])/1.85      #テンポ調整
        time_buf += long(notes[-1])/1.85
        ...
        #note_on event
        #if you want to chage pitch, do like below
        #player.write([[[0x90 + int(notes[1]),int(notes[2])+12,int(notes[3])],time_stamp]])
        if notes[0] == 'non':
