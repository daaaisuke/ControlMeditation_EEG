...
while EndTrigger:
    for notes in lines:
        time_stamp += long(notes[-1])/1.8
        Ftime_stamp += long(notes[-1])/1.8
        if Ftime_stamp >= 5000:
            time.sleep(5)
            Ftime_stamp = 0
        #note_on event
        if notes[0] == 'non':
            player.write([[[0x90 + int(notes[1]),int(notes[2]) - 12,int(notes[3])],time_stamp]]) #信号ステータス,ピッチ,ベロシティ,タイムスタンプ
    
        #note_off event
        elif notes[0] == 'nof':
            player.write([[[0x90 + int(notes[1]),int(notes[2]) - 12,0],time_stamp]])
